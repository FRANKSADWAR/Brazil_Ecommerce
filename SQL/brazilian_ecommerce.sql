...
WITH product_category_purchases AS (
    SELECT
        itm.order_id,
        itm.product_id,
        itm.price,
...
category_revenue AS (
    SELECT
        ...
        DENSE_RANK() OVER (ORDER BY SUM(price) DESC) AS category_rank
    FROM product_category_purchases
    GROUP BY product_category
)
SELECT 
    product_category,
    revenue_per_product_category,
    total_revenue,
    ROUND((revenue_per_product_category / total_revenue), 4) AS revenue_percentage
FROM category_revenue
WHERE category_rank <= 10
ORDER BY revenue_per_product_category DESC;


--- Least performing products in the Brazil e-commerce market
WITH product_category_purchases AS (
    SELECT
        itm.order_id,
        itm.product_id,
...
category_revenue AS (
    SELECT
        ...
        SUM(price) AS revenue_per_product_category,
        SUM(SUM(price)) OVER () AS total_revenue,
        DENSE_RANK() OVER (ORDER BY SUM(price) ASC) AS category_rank
    FROM product_category_purchases
    GROUP BY product_category
)
SELECT 
...
-- Check contacts not updatedWITH
  customer_list AS (
    SELECT DISTINCT
      (so.customer_name) AS customer_name,
      so.company,
      cu.customer_group,
      cu.customer_primary_contact,
      cu.mobile_no,
      cu.email_id,
      cu.registry_id
    FROM
      `tabSales Order` AS so
      INNER JOIN `tabCustomer` AS cu ON so.customer = cu.name
    WHERE
      so.transaction_date >= '2024-01-01'
      AND so.company = 'agriBORA'
      AND cu.customer_group = 'Hub'
  ),
  users AS (
    SELECT
      us.name,
      ...
      us.first_name,
      us.last_name,
      us.role_profile_name,
      us.phone
    FROM
      `tabUser` AS us
    WHERE
      us.role_profile_name = 'Hub Manager'
  )
SELECT
    cl.customer_name,
    cl.customer_primary_contact,
    cl.mobile_no,
    cl.registry_id,
    users.name,
    users.email,
